...
export default function TrajectorySidePanelMaxSpeed({className, trajectoryData, frictionData, frictionInTime, vectorsInTime, hoveredPoint}) {

    return <div className={className + " bg-[hsl(218,80,10)] rounded-[2rem] p-4 flex"}>
        <div className="m-auto w-full h-full flex flex-col">
            {hoveredPoint !== null ? (
                <>
                    <HoveredPointDataMaxSpeed className=" w-full" trajectoryData={trajectoryData} frictionData={frictionData} hoveredPoint={hoveredPoint} frictionInTime={frictionInTime} vectorsInTime={vectorsInTime}/>
