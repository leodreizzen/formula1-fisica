...
export default function DriverSelector({className}) {
    ...
    return (<div className={"text-white " + className}>
        ...
        <DriverSelect className={"block bg-[#010409] mx-auto w-max min-w-64 mb-5 border border-gray-400 rounded-md"} drivers={drivers ?? []} value={currentDriver ? currentDriver.driverNumber : ""} disabled={drivers !== null ? null : true} onChange={onDriverSelectionChange}/>
    </div>);
}