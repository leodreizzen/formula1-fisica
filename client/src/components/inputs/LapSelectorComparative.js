...
export default function LapSelector({className, secondaryLapsCount}) {
    ...
    useEffect(() => {
        if(currentLap !== null) {
            if (currentLap <= minLapsCount) {
                setCurrentLapInput(currentLap);
            } else {
                setCurrentLapInput(minLapsCount);
            }
        } 
    }, [currentLap, secondaryLapsCount]);
    ...
    return (lapCount !== null?
            ...
            <div className={"flex flex-col sm:flex-row items-center border-2 border-gray-400 rounded-md pl-2 mt-4 " + className}>
                <>
                    ...
                    {currentLap !== null && <Pagination variant="outlined" count={minLapsCount} page={currentLap} onChange={onPaginationChange}
                                className="grow"/>}
                </>
            </div>
