...
export default function AccelerationsPanel({className, drivers, selectedDriver, lap, onSelectedDriverChange}){

    const {year, round, session} = useContext(SessionDataContext);

    const [accelerationData, accelerationDataLoading] = useGetAcceleration(year, round, session, selectedDriver, lap);
